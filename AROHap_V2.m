...
for d=1:Num_paras
    ...
    for i=1:Num_exps  %for each instance
        ...
        while ing<=ng && BestScore>0
            ...
            %[BudScore]=FitAroHap4_2alpha(bud,budfrags);
            %MinScore=min(ParentScore,BudScore);
            if ParentScore>BudScore
                CHR=bud;
                ParentScore=BudScore;
                %CompactFrags=budfrags;
            end
        ...
        H_rec=MakeHapbyMajority(CHR,M);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %H_rec=MakeHapbyMajority(init,M);
        %H_rec=ImproveHap(H_rec,init,M);%%%%%
    ...
    str=['AROHap_',D1(d).name,'_RR_Mask.txt'];
    fprintf('\n%s',str);
    fid=fopen(['AROHap_',D1(d).name,'_RR_Mask.txt'],'w+');
    for j=1:Num_exps
